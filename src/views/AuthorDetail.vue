...
export default {
  ...
  setup() {
    ...
    // Get author slug from route params
    ...
    // Filter author data based on slug
    const findAuthorBySlug = (data, slug) => {
      return data.find(item => 
        (item.id).toLowerCase() + "-quotes" === slug.toLowerCase()
      )
    }

    // Set page meta data
    const setPageMeta = (author) => {
      if (author) {
        document.title = `${author.name} ${author.surname} Quotes`
        
        // Update meta description
        const metaDescription = document.querySelector('meta[name="description"]')
        if (metaDescription) {
          metaDescription.setAttribute('content', author.en_meta_desc || '')
        }
        
        // Update Open Graph tags
        const ogTitle = document.querySelector('meta[property="og:title"]')
        if (ogTitle) {
          ogTitle.setAttribute('content', `${author.name} ${author.surname} Quotes`)
        }
        
        const ogDescription = document.querySelector('meta[property="og:description"]')
        if (ogDescription) {
          ogDescription.setAttribute('content', author.en_meta_desc || '')
        }
        
        const ogUrl = document.querySelector('meta[property="og:url"]')
        if (ogUrl) {
          ogUrl.setAttribute('content', `https://wordsfromlife.com/authors/${author.id}-quotes`)
        }
        
        const ogImage = document.querySelector('meta[property="og:image"]')
        if (ogImage) {
          ogImage.setAttribute('content', author.img)
        }
      }
    }

    // Add breadcrumb schema
    const addBreadcrumbSchema = (author) => {
      if (author) {
        const breadcrumbSchema = {
          ...
          "itemListElement": [{
            ...
            "@type": "ListItem",
            "position": 2,
            "name": `${author.name} ${author.surname} Quotes`
          }]
        }
    ...
    // Load author data on component mount
    onMounted(async () => {
      try {
        loading.value = true
